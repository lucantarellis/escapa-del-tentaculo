...
3. **Arranque.** `Main` elimina el menú y llama a `LevelController.begin()`: el nivel se reanuda (`PROCESS_MODE_INHERIT`) y se llama a `GameManager.start_run(seed)` (`PLAYING`). La cuenta de `start_delay` de la cámara empieza recién ahí.
...
| `LevelController.begin()` | Reanuda el nivel y llama a `GameManager.start_run()`. Idempotente |
...
2. Clic (o Enter/Espacio) en JUGAR: el menú se disuelve y luego la cámara empieza a subir. Pulsar varias veces rápido no rompe nada.
