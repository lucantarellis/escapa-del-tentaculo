...
`scenes/main/Main.tscn` es la escena principal (`run/main_scene`): muestra la pantalla de título (nombre del juego y un botón **JUGAR**) sobre el nivel ya armado y quieto (`scenes/levels/Level.tscn`). Al pulsar JUGAR el título se disuelve, una escotilla golpea tres veces (vibración de cámara y gas; ver `docs/mecanicas/intro-escotilla.md`) y arranca el gameplay; R reinicia directo al juego, sin intro. Ver `docs/mecanicas/pantalla-titulo.md`.
